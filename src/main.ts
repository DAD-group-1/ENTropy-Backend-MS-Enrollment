...
async function bootstrap() {
  const app = await NestFactory.create(AppModule, {
    logger: createWinstonLogger('enrollment-service', 'info'),
  });
  const configService = app.get(ConfigService);

  const microserviceHost = configService.get<string>('HOST', '0.0.0.0');
  const microservicePort = configService.get<number>('PORT', 3005);
  ...
  new Logger('Bootstrap').log(
    `Enrollment microservice started on ${microserviceHost}:${microservicePort}`,
  );
}
bootstrap();
